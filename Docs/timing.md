...
| **Button Debounce**| 50 ms | SysTick Timestamp | Software filtering of signal noise. |
| **PWM Ramp Step** | 20 ms | SysTick Timestamp | 5% duty change every step for smooth motor simulation. |
| **Cabin Button Scan**| 10 ms | SysTick Timestamp | Periodic non-blocking polling (replaces EXTI). |
...
* **Scan 10ms**: Ensures human button presses are captured with high temporal resolution while resolving the STM32 EXTI multiplexer conflict.
