...
void SpiProto_Pack(const SpiFrameData *data, uint8 *buf) {
    ...
    buf[2] = (uint8)((data->direction << 4) | (data->cabinRequests & 0x0F));
    buf[3] = data->assignedCalls;
    buf[4] = data->hallCalls;
    buf[5] = data->flags;
    buf[6] = data->sequence;
...
boolean SpiProto_Unpack(const uint8 *buf, SpiFrameData *data) {
    ...
    data->cabinRequests =  buf[2]       & 0x0F;
    data->assignedCalls =  buf[3];
    data->hallCalls     =  buf[4];
    data->flags         =  buf[5];
    data->sequence      =  buf[6];
